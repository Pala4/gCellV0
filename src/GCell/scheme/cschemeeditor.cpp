    ...
    case CSchemeEditor::MoveSelectMode:
        if (event->buttons() & Qt::LeftButton) {
            if (itemAt(event->pos()) != nullptr) {
                if (dynamic_cast<CAlgorithm*>(itemAt(event->pos())) != nullptr) {
                    m_algorithmMover.release();
                    if (scene() != nullptr)
                        m_algorithmMover.addElements(scene()->selectedItems(),
...
{
    QString title = scheme() ? scheme()->caption() : QString();
