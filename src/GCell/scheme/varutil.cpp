...
QString variantToString(const QVariant &v)
{
	...
	switch (v.type()) {
		...
		case QVariant::PointF:
		{
			...
			result += QString("%1").arg(p.x());
			result += QLatin1Char(' ');
			result += QString("%1").arg(p.y());
			result += QLatin1Char(')');
			break;
		}
	...
	if(s.startsWith(QLatin1Char('@')))
	{
		if(s.endsWith(QLatin1Char(')')))
		{
			...
			else if(s.startsWith(QLatin1String("@Rect(")))
			{
				...
				if(args.size() == 4) return QVariant(QRect(args[0].toInt(), args[1].toInt(), args[2].toInt(), args[3].toInt()));
			}
			else if(s.startsWith(QLatin1String("@Size(")))
			{
				QStringList args = splitArgs(s, 5);
				if(args.size() == 2) return QVariant(QSize(args[0].toInt(), args[1].toInt()));
			}
			else if(s.startsWith(QLatin1String("@Point(")))
			{
				QStringList args = splitArgs(s, 6);
				if(args.size() == 2) return QVariant(QPoint(args[0].toInt(), args[1].toInt()));
			}
			else if(s.startsWith("@PointF("))
			{
				QStringList args = splitArgs(s, 7);
				if(args.size() == 2) return QVariant(QPointF(args[0].toDouble(), args[1].toDouble()));
			}
#endif
		}
		...
		if(s.startsWith(QLatin1String("@@"))) return QVariant(s.mid(1));
	}
